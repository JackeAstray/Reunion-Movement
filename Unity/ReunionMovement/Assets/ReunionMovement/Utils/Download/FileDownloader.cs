...
namespace ReunionMovement.Common.Util.Download
{
    ...
    public class FileDownloader : IDownloader
    {
        ...
        // 用于零分配 FIFO 消费的偏移量（替代 Skip().ToArray()）
        ...
        /// <summary>
        /// 下载器的URI（带超时保护，默认超时由 Timeout 属性 × 文件数决定，最少 30 秒）
        /// </summary>
        /// <returns></returns>
        public override async UniTask<bool> Download()
        {
            ...
            // 带超时的等待循环：最多等待 (Timeout * 文件数 * 2) 秒，最少 30 秒
            ...
            while (Downloading && !(DidError && !ContinueAfterFailure))
            {
                ...
                {
                    Log.Error("下载超时：已等待 {0} 秒，尚有 {1} 个文件未完成。强制取消。", maxWaitSeconds, NumFilesRemaining);
                    await Cancel();
                    break;
                }
        ...
        /// <summary>
        /// 派遣下载器（使用偏移索引替代 Skip+ToArray，避免每次调度分配数组）
        /// </summary>
