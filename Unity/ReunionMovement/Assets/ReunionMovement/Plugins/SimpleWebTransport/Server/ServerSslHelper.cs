    ...
    internal class ServerSslHelper
    {
        ...
        public ServerSslHelper(SslConfig sslConfig)
        {
            ...
            if (config.enabled)
            {
                ...
                Log.Info($"[SWT-ServerSslHelper]: 已加载 SSL 证书 {0}，到期时间 {1}", certificate.Subject, certificate.GetExpirationDateString());
            }
        }
        ...
        internal bool TryCreateStream(Connection conn)
        {
            ...
            if (config.enabled)
            {
                ...
                {
                    Log.Error("[SWT-ServerSslHelper]: 创建 SSLStream 失败: {0}", e.Message);
                    return false;
                }
            }
            else
